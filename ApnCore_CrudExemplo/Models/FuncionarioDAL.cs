...
namespace ApnCore_CrudExemplo.Models
{
    ...
    {
        string connectionString = @"Password=millennium_qas3;Persist Security Info=True;User ID=millennium_qas3;Initial Catalog=millennium_qas3;Data Source=172.26.225.252";
        ...
        public IEnumerable<Funcionario> GetAllFuncionarios()
        {
            ...
            {
                SqlCommand cmd = new SqlCommand("SELECT FuncionarioId, Nome,Cidade, Departamento,Sexo from dbo.crud_Funcionarios", con);
        ...
        public void AddFuncionario(Funcionario funcionario)
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                string comandoSQL = "Insert into dbo.crud_Funcionarios (Nome,Cidade,Departamento,Sexo) Values(@Nome, @Cidade, @Departamento, @Sexo)";
        ...
        public void UpdateFuncionario(Funcionario funcionario)
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                string comandoSQL = "Update dbo.crud_Funcionarios set Nome = @Nome, Cidade = @Cidade, Departamento = @Departamento, Sexo = @Sexo where FuncionarioId = @FuncionarioId";
                SqlCommand cmd = new SqlCommand(comandoSQL, con);
                cmd.CommandType = CommandType.Text;
        ...
        public Funcionario GetFuncionario(int? id)
        {
            ...
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                string sqlQuery = "SELECT * FROM dbo.crud_Funcionarios WHERE FuncionarioId= " + id;
        ...
        public void DeleteFuncionario(int? id)
        {
            using (SqlConnection con = new SqlConnection(connectionString))
            {
                string comandoSQL = "Delete from dbo.crud_Funcionarios where FuncionarioId = @FuncionarioId";
